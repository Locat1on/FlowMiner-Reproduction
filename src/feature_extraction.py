...
def enhance_with_cross_features(df: pd.DataFrame, top_k: int, bin_count: int) -> pd.DataFrame:
    ...
    # 必须排除 label，否则会发生严重的数据泄露（label 参与特征构造）
    protected_cols = {"flow_id", "direction", "timestamp", "label"}
...
def run_pipeline(
    ...
    if no_enhance:
        print("[FlowMiner] Step 4/4: skipping feature enhancement (copying raw nodes) ...")
        nodes.to_csv(enhanced_path, index=False)
        print(f"[FlowMiner] Step 4/4 done: raw features saved to {enhanced_path}.")
    else:
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--no-enhance", action="store_true", help="Skip feature enhancement (binning, cross features).")
    return parser
